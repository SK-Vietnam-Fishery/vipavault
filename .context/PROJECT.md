...
- **Login V1 (SIMPLE):** unlock — master password only; UI — vault + operator email label + password (`operator_email` plaintext in `app_settings.json`, backend Rust, regex format on first entry, `whoami` fallback — **not** email auth). Xem `docs/technical-decisions.md` §3.1. Roles qua `app_settings.json`. Share Package + email gate — FULL defer.
...
- KDBX export/import optional — Phase 2+ interop thô; **không** thay Share Package (KeePass không có activation-code claim / relational subset). Nợ kỹ thuật: questionnaire §FULL `TD-SHARE-*`.